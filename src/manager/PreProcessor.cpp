...
std::vector<ImageWrapper> PreProcessor::processImages(std::map<QString, QImage> imgMap) {
    ...
    for (auto it = imgMap.begin(); it != imgMap.end(); ++it) {
        ...
        std::cout << meanR << ":" << meanG << ":" << meanB << std::endl;
        ...
        /*for (int bgr = 0; bgr < 3; bgr++) {
            for (int y = 0; y < height; y++) {
                for (int x  = 0; x < width; x++) {
                    switch (bgr) {
                        case 0: bitmap[bgr + y *height + x * width] -= (meanB)*3;
                            break;
                        case 1: bitmap[bgr + y *height + x * width] -= (meanG)*3;
                            break;
                        case 2: bitmap[bgr + y *height + x * width] -= (meanR)*3;
                            break;
